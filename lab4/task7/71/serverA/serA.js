...
function insertRecord(name, cost)
{
    ...
    if (isUnique(name)) {
        ...
        let content = JSON.parse(fs.readFileSync("cars.txt", "utf-8"));
        content.push(record);
        fs.writeFileSync("cars.txt", JSON.stringify(content));
    }
    else {
...
app.post("/insert/record", function(request, response) {
    loadBody(request, function(body) {
        ...
        let res = insertRecord(name, cost);
...
{
    let recordsArrJS = fs.readFileSync("cars.txt", "utf-8");
